...
def test_base_template_centers_footer_when_footer_right_empty():
    ...
    template_text = (random_paro_render.TEMPLATE_DIR / "base.html").read_text(encoding="utf-8")

    assert "margin: 14px 38px 0; padding: 8px 0 10px;" in template_text
    assert "line-height: 14px;" in template_text
...
def test_draw_result_template_uses_120px_pair_tiles():
    template_text = (random_paro_render.TEMPLATE_DIR / "draw_result.html").read_text(encoding="utf-8")

    assert ".tile {" in template_text
    assert "width: 120px;" in template_text
    assert "height: 120px;" in template_text
    assert ".topbar {" in template_text
