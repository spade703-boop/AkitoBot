...
- 保证礼「自己产的彰冬饭」「彰冬婚礼邀请函」一旦抽中**必定惊喜升级**（不暴击不失败、固定加满羁绊）；婚礼邀请函是终极礼（1112 消耗 / 基础羁绊 +819），每位群友首次送出时另有一次 +495 纪念加成、合计 +1314；同一对关系无论方向只能出现一次邀请函，之后互送会自动从其他礼物中重抽
...
- RPG 的成长硬约束已单独整理在 `features/rpg/GROWTH_BASELINE.md`；当前默认把签到经验收在轻量背景奖励区间（10 起步，连签最高 20），主成长仍然来自打怪推进
...
│           ├── hunt.py             # 打怪指令 + 战斗结算 + 援护/追击 + 单人/双人小奇遇
│           ├── boss.py             # 世界BOSS刷出 / 强制开启 / 查询 / 独立装备攻击 / 贡献结算
│           ├── team.py             # 组队@某人 指令 + 负羁绊事件 + 双人小奇遇 + 失败援护拉回
