...
- 送礼按权重触发随机事件：普通 / 暴击（羁绊翻倍）/ 回礼（对方随机回赠 5 种彰冬周边之一，礼尚往来额外加羁绊、稀有档还退部分积分，全部可在 `return_gifts` 配置增删调）/ 失败（不涨羁绊）/ 意外（9 种小概率：加赠·稀有·手写卡·被夸·上头·迟到·翻车·撞款·寄丢，偏正向小惊喜，部分返还积分，全部可在 `mishaps` 配置增删调）
