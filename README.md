...
### RPG 冒险系统（`今日打怪` / `组队` / `强化今日装备` / `我的背包` / `群排行榜`）
...
| `组队@某人` | 拉群友合力打怪，直接 @ 即组队；对方需今日装备完好（未签到/已损坏则直接拒绝）；成功率绑羁绊等级（满羁绊≈必成）；成功双方各得经验积分掉落，失败退化为发起人单刷（队友无损） |
| `强化今日装备` | 花积分把今天装备提一提；分段收费 `[60,150,300]`，每日限 3 次，次日重置 |
| `购买装备` | 装备损坏后花 100 积分买一套替换装（打怪积分减半），每日可多次购买 |
