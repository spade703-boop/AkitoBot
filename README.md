...
| `今日打怪` | 用今日装备出门打怪，胜败均有经验和积分、概率掉道具；遇精英怪/今日增益藏着不外显；低等级先碰温和怪；打完装备损耗，每日一次；战后极低概率刷出世界 BOSS |
| `组队@某人` | 拉群友合力打怪，直接 @ 即组队；对方需今日装备完好（未签到/已损坏则直接拒绝）；成功率绑羁绊等级（满羁绊≈必成，负羁绊会小幅降低成功率）；成功结队后有战力/掉落协作加成，并小幅增长双方羁绊；负羁绊还可能触发摩擦或缓和事件；失败则退化为发起人单刷 |
