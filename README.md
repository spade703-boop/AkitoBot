...
| `组队@某人` | 拉群友合力打怪，直接 @ 即组队；对方需今日装备完好（未签到/已损坏则直接拒绝）；成功率绑羁绊等级（满羁绊≈必成）；成功双方各得经验积分掉落；羁绊不够则退化为发起人单刷 |
| `世界BOSS` | 查看当前世界 BOSS 状态、剩余生命和贡献榜；若当天没有刷出会直接提示 |
| `攻击世界BOSS` | 用今天的装备单人攻击世界 BOSS，返回本次精确伤害；奖励按最终贡献结算 |
| `组队世界BOSS@某人` | 邀请 1 名群友双人挑战世界 BOSS；成功则双方分别计算伤害并共同进榜，失败则退化为发起人单打 |
...
- 世界 BOSS 只会在普通打怪后以 `0.1%` 概率出现；强度按近 7 日活跃签到人数生成，奖励只有经验和积分，不计入普通战绩
- 称号随等级分档，与战绩搭配供攀比，不刷屏
- 组队把「送礼攒羁绊 → 拉人成功率」串成了社交闭环：羁绊越深越容易拉动，反过来促进送礼
- `重置RPG功能` 会为今天已经完成 RPG 签到的人重发今日装备，不改运势、连签和其他状态（**超管**）
- 数值 / 野怪 / 掉落 / 文案全部在 `data/content/rpg_config.json`（热重载），缺省值内置于代码
- 睡眠时段 0–6 点拦截写操作（今日打怪/组队/强化今日装备），查询类照常；超管不受限
...
│           ├── hunt.py             # 打怪指令 + 战斗结算
│           ├── boss.py             # 世界BOSS刷出 / 查询 / 攻击 / 贡献结算
│           ├── team.py             # 组队@某人 指令
│           ├── smith.py            # 强化 / 重置RPG功能（积分出口 + 超管测试辅助）
