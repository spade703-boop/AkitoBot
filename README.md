...
| 指令 | 说明 |
|------|------|
| `签到` | 每天 1 次领取积分（赚取入口），随机 50–100 分；重复签到静默不应答 |
...
- 礼物 6 档，按「心意 / 稀有度」递增（彰冬无料 → 彰冬谷子 → 彰冬豆豆眼 → 同人本 → 约稿点图 → 自己产的彰冬饭）；越贵的礼基础羁绊越高，攒钱送贵礼更划算
- 送礼按权重触发随机事件：普通 / 暴击（羁绊翻倍）/ 回礼（对方回赠一份谷子）/ 失败（不涨羁绊）/ 意外（9 种小概率：加赠·稀有·手写卡·被夸·上头·迟到·翻车·撞款·寄丢，偏正向小惊喜，部分返还积分，全部可在 `mishaps` 配置增删调）
...
- 0–6 点（北京时间）小彰睡觉，签到 / 送礼会收到固定回复、暂不结算（接入全局 `is_sleeping()`）；查询类（积分 / 亲密度 / 排行）照常。超管不受睡眠限制，便于测试
- 礼物档位 / 事件权重 / 签到积分 / 羁绊等级 / 全部文案都在 `data/content/gift_config.json`（可热重载，仓库附带模板），缺省值同时内置于代码
