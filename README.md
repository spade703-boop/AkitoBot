...
群友互送的轻量社交小游戏：签到攒积分，`送礼 @对方` 由系统从你买得起的礼物里随机送一份，累积两个群友之间的「同好羁绊」。礼物与文案均为彰冬同人圈主题。

| 指令 | 说明 |
|------|------|
| `签到` | 每天 1 次领取积分（赚取入口），随机 30–80 分 |
| `送礼 @对方` | 每天 1 次，从你当前积分买得起的礼物里随机送一份给对方 |
| `我的积分`（`积分`） | 查看自己的积分与今日可签到 / 可送礼状态 |
| `礼物列表`（`送礼帮助`） | 查看礼物档位与价位 |
| `亲密度 [@对方]` | 查看与某人的同好羁绊，或自己羁绊最高的几位 |
| `亲密度排行` | 本群同好羁绊对子排行 |
| `重置送礼` | 清空本群送礼 / 积分 / 羁绊数据（**超管**） |

- 礼物 6 档，按「心意 / 稀有度」递增（彰冬谷子 → 豆豆眼 → 无料 → 同人本 → 约稿点图 → 自己产的彰冬饭）；越贵的礼基础羁绊越高
- 送礼按权重触发随机事件：普通 / 暴击（羁绊翻倍）/ 回礼（对方回赠一份谷子）/ 失败（不涨羁绊）/ 意外（快递翻车、礼物过敏，均返还一半积分）
- 顶档「自己产的彰冬饭」一旦抽中**必定惊喜升级**，固定加满羁绊
- 仅白名单群可用；亲密度按群隔离；每人每天各 1 次签到与送礼，基于日期自动跨天刷新
- 礼物档位 / 事件权重 / 签到积分 / 全部文案都在 `data/content/gift_config.json`（可热重载），缺省值内置于代码
...
| `gift_config.json` | 送礼系统配置（礼物档位 / 事件权重 / 签到积分 / 播报文案，可热重载；缺省值内置于代码） |
