...
在送礼社交之上的轻量群文字 RPG。与送礼共享同一套积分账本，签到同时触发两种结算（积分 + 经验 + 今日装备）。设计原则是：**平时走轻量个人挑战线，低频再用世界 BOSS 承接群体参与感。**

| 指令 | 说明 |
|------|------|
| `签到` | （送礼系统提供）领积分 + 经验 + 暗掷当日运势（隐藏）+ 今日装备（战力随等级涨、随机浮动）；连签有额外经验递增 |
| `今日打怪` | 用今日装备出门打怪，胜败均有经验和积分、概率掉道具；遇精英怪/今日增益藏着不外显；低等级先碰温和怪；打完装备损耗，每日一次；单刷还有低概率触发追击/援护转场；战后极低概率刷出世界 BOSS |
| `组队@某人` | 拉群友合力打怪，直接 @ 即组队；对方需今日装备完好（未签到/已损坏则直接拒绝）；成功率绑羁绊等级（满羁绊≈必成，负羁绊会小幅降低成功率）；成功结队后有战力/掉落协作加成，并小幅增长双方羁绊；负羁绊还可能触发摩擦或缓和事件；失败通常退化为发起人单刷，但也有低概率被援护拉回成立 |
| `世界BOSS` | 查看当前世界 BOSS 状态、剩余生命和贡献榜；若当天没有刷出会直接提示；若昨天那只没打完，会先按贡献发放补偿再清场 |
| `攻击世界BOSS` | 世界 BOSS 是独立的群挑战线；今天签到过就能生成一套临时世界BOSS装备。每只 BOSS 每人可出手 1 次，返回本次精确伤害；击败后按最终贡献结算，未击败则隔天按进度折算补偿 |
| `组队世界BOSS@某人` | 邀请 1 名已签到群友双人挑战世界 BOSS；成功则双方分别计算伤害并吃到小额协作伤害加成，还会给双方结算一笔很轻的小额羁绊成长；失败则退化为发起人单打 |
| `强化世界BOSS装备` | 世界 BOSS 出现后，可单独强化这套临时装备；与普通今日装备分开计算，同样是 `[30,60,90]` 三段、最多 3 次 |
| `强制开启世界BOSS` | 无视概率和活跃人数门槛，立刻在本群刷出一只世界 BOSS；若今天已有 BOSS，则只返回当前状态（**超管**） |
| `强化今日装备` | 花积分把今天装备提一提；分段收费 `[30,60,90]`，每日限 3 次，次日重置 |
| `购买装备` | 装备损坏后花 100 积分买一套替换装（每天限 1 次，打怪经验和积分减半） |
| `我的角色` | 看等级/称号/战绩/普通装备状态/世界BOSS状态/积分/背包，以及已获得的世界BOSS收藏；战力为隐藏值不显示 |
| `群排行榜` | 本群冒险者经验 Top 10，纯文字不 @ 不出图 |
| `我的背包` | 列出道具与数量 |
| `使用 [道具名]` | 用消耗品；礼物券类道具需 `使用 道具名 @某人`（送对应礼物、走完整随机事件） |
| `冒险帮助` | 列出以上所有指令 |

- 角色对外的唯一数值是「等级」；战力/运势/今日增益均为隐藏值（暗中影响打怪），只在面板和播报中给模糊反馈
- 当前把 `Lv30` 视作这一阶段的成长终点；默认曲线刻意放长到接近半年，打赢推进更快，打输也有稳定保底经验
- 野怪基础 `power_req` 暂时不抬，主要通过 `combat.encounter_brackets` 后移中阶怪出场，避免慢成长后前期过早撞上座狼、食人魔
- 普通打怪额外挂了一层轻量特判：单刷胜利时，低概率触发彰人追击补一笔额外经验/积分；单刷失败时，可能被冬弥援护或彰冬联携拉回成功；组队失败则有低概率在改单刷前被支援拉回成立
- 世界 BOSS 只会在普通 `今日打怪` / `组队@某人` 后以 `1%` 概率出现；强度按近 7 日活跃签到人数生成，12 人后血量继续软扩容，但奖励扩容更慢；基础奖励仍以经验和积分为主，不计入普通战绩
...
- 称号档位改得更密，低中段升级时会更频繁地给到阶段反馈；依然只在查询时展示，不刷屏
...
│           ├── hunt.py             # 打怪指令 + 战斗结算 + 援护/追击
│           ├── boss.py             # 世界BOSS刷出 / 强制开启 / 查询 / 独立装备攻击 / 贡献结算
│           ├── team.py             # 组队@某人 指令 + 失败援护拉回
│           ├── smith.py            # 今日装备强化 / 世界BOSS装备强化 / 购买装备 / 重置RPG功能
...
| `rpg_config.json` | RPG 全量配置（战斗/运势/强化/掉落/精英/世界BOSS/野怪/道具/文案/错误信息，热重载；仓库附带模板，缺省值内置于代码） |
