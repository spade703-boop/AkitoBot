...
覆盖台账把已抓取草稿、已发布人工事件和手动加入的待办 URL 合并到同一个列表，并显示 `todo`、`draft`、`approved`、`published`、`rejected`、`revision_pending` 状态。默认维护视图隐藏 `rejected`；它们只是避免重复处理的历史归档，不参与覆盖分类、评测和进度统计。分类中的“原剧情整页角色范围”描述整页原剧情动作里出现的角色，而不是“实际写入的目标片段包含谁”；`source_speakers` 记录整页说话人，`target_speakers` 记录目标片段说话人，后者默认只有彰人/冬弥。AI 只为 `published` 剧情建议时间阶段、事件类型、场景角色范围和召回评测问题，不会自动确认分类，也不会直接修改正式评测集；必须在网页中人工核对并批准。

当前时间阶段词表覆盖从初遇到 RUSH BEATS 的完整推进：`RUSH BEATS目标确立` 是确定下一目标，`RUSH BEATS筹备（日本）` 是出发前的资金/资格/英语/训练准备，`赴美/美国筹备（RUSH BEATS）` 是已到美国但正式比赛尚未开始，随后才是 `RUSH BEATS比赛进行中` 和 `RUSH BEATS赛后`。美国活动、适应环境或与对手交流的剧情应归入美国筹备，不应仅因出现“比赛”一词就归入比赛进行中。

召回评测不是逐句对白清点，而是以已发布的共同经历事件为基本单位；一个事件若包含多个独立互动单元，可用多组问题覆盖，至少包含不同问法、相邻剧情干扰和事实错误负例。
