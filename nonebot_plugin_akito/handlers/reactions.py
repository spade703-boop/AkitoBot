...
async def get_toya_location_reply(user_mem: dict, user_question: str = "冬弥现在在哪？") -> str:
    ...
    if 0 <= now.hour < 6:
        ...
        grant_safety_pass(5)
        return "（正在熟睡中，完全没有听到你在问冬弥的事……）zzZ"
...
async def _(bot: Bot, event: NoticeEvent):
    ...
    now = datetime.datetime.now(TZ_CN).time()
    if datetime.time(0, 0) <= now < datetime.time(6, 0):
        grant_safety_pass(5)
        await poke.finish("（这只松饼正在睡觉，没有理会你的戳）")
        return

    now_full = datetime.datetime.now(TZ_CN)
    get_daily_activity(now_full.hour, now_full.weekday())
...
async def _(bot: Bot, event: Event):
    ...
    now = datetime.datetime.now(TZ_CN)
    if not (0 <= now.hour < 6) or time.time() < get_safe_until() or time.time() - get_last_complaint() < 10:
        return
    ...
    try:
        group_id = getattr(event, "group_id", None)
        if group_id: await bot.send_group_msg(group_id=group_id, message=random.choice(REACTIONS_DB.get("complaints", ["……困……"])))
