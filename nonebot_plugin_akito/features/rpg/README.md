...
- **组队**（可选）→ 打怪前 `组队@某人` 合力，羁绊越深越容易拉动；成功结队后会给小额羁绊成长，串起「送礼攒羁绊 → 组队 → 再磨合」闭环
...
| `组队@某人` | 拉群友合力打怪，直接 @ 即组队；成功率绑羁绊等级（满羁绊≈必成，负羁绊会小幅降低成功率）；成功结队后结算战力/掉落协作加成，并给该 pair 小额羁绊成长；负羁绊还可能触发摩擦或缓和事件；失败退化为发起人单刷（队友无损） |
...
├── team.py                   `组队@某人` 指令：羁绊定成功率、负羁绊事件/小额羁绊增长、失败退化单刷（复用 hunt 结算）
...
- 组队：`team` → `game_store._get_intimacy` + `gift._bond_level`（定成功率）→ 负羁绊额外事件 / 每日限次羁绊增长 → `hunt._settle_coop`（成）/ `hunt._settle_solo`（败，单刷）。
...
+ 指令行为（签到钩子含连签/打怪含积分·精英·今日增益/战绩计数/强化与重置 RPG / 面板含称号战绩/排行榜排序·过滤·空榜/背包/普通组队成功·失败退化单刷/负羁绊缓和事件/世界 BOSS 查询·单人攻击·双人攻击·击杀结算）。
