...
- **签到** → 领积分 + 经验（+ 连签递增）+ 今日装备
- **打怪** → 用今日装备挑战野怪，赢经验、积分与掉落；偶遇**精英怪**、撞上**今日增益**（藏着不外显）；单刷还会低概率触发追击或援护转场
- **组队**（可选）→ 打怪前 `组队@某人` 合力，羁绊越深越容易拉动；成功结队后会给小额羁绊成长，串起「送礼攒羁绊 → 组队 → 再磨合」闭环；失败通常退化单刷，但也可能被援护拉回成立
...
| `今日打怪` | 用今日装备出门打怪，胜败均有经验和积分、概率掉道具；遇精英怪/今日增益藏着不外显；低等级先碰温和怪；打完装备损耗，每日一次；单刷还会低概率触发追击/援护转场；战后额外判定是否刷世界 BOSS |
| `组队@某人` | 拉群友合力打怪，直接 @ 即组队；成功率绑羁绊等级（满羁绊≈必成，负羁绊会小幅降低成功率）；成功结队后结算战力/掉落协作加成，并给该 pair 小额羁绊成长；负羁绊还可能触发摩擦或缓和事件；失败通常退化为发起人单刷（队友无损），但也有低概率被援护拉回成立 |
...
普通打怪额外挂了一层轻量战斗特判：单刷胜利时，低概率触发彰人追击补一笔额外经验/积分；单刷失败时，可能被冬弥援护或彰冬联携拉回成功；组队失败则有低概率在改单刷前被支援拉回成立。
