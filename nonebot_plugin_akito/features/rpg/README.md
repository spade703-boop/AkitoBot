...
| `打怪` | `打野`、`挑战` | 用今日装备挑战随机野怪 | 需「今日装备未损坏」（即今天签到过且还没打）。判定：今日装备战力 × 随机系数 × 隐藏运势系数 × 随机事件，与野怪 `power_req` 比胜负；**胜多输少**经验（按等级）+ 少量积分（`challenge.win_points/lose_points`）、按掉率出道具。**变数（藏着不外显）**：小概率遇**精英怪**（更难打、胜则经验/掉落翻倍），以及**今日增益**（按日期全群一致，如「掉落翻倍日 / 经验涌动日」，生效才播报）。低等级前几天会先落在更温和的遭遇池里，避免“一天一把”时连续挫败。打完**装备损坏**，次日签到再领。每日一次。 |
...
| `强化` | `锻造`、`强化装备` | 花积分提升今日装备战力 | 主要的积分出口。优先按 `forge.costs` 分段收费（未配时回退 `forge.cost_base × n`）、每日上限 `forge.max_per_day`、次日随装备重置。第 1 次强化适合作为日常消耗，后续强化则更偏囤分玩家的额外追求。 |
