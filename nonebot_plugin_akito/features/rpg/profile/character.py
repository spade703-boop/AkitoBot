...
from ..inventory.inventory import _active_battle_debuff, _active_battle_supply
...
from ..world_boss.logic import _active_world_boss, _ensure_boss_participant
...
_HELP_ITEMS = [
    ...
    {"command": "今日打怪", "description": "进行一次普通个人挑战，结算经验、积分与随机掉落；每日装备默认可使用一次。", "sections": [
        {"title": "战后小奇遇", "lines": [
            "个人挑战：普通个人挑战结算后有 6% 概率触发。",
        ...
        {"title": "小奇遇奖励", "lines": [
            ...
            "遗失钱袋：提供少量积分。",
            "破旧宝箱：随机获得以下一种道具。",
            "破旧积分卡：抽到后立即获得 5 积分，不进入背包。",
            "破旧经验券：抽到后立即获得 10 经验，不进入背包。",
    ...
    {"command": "开启冒险补给 [数量]", "description": "每周最多开启七次；可一次开启本周剩余次数，费用按所在档位累加，每次获得 30 经验与一件道具。", "sections": [
        {"title": "战备使用规则", "lines": [
            "战备进入背包后需主动使用，所有战备均不作用于世界BOSS。",
            "战备、护符和大葱味蛋糕不能批量使用；需要逐个启用或赠送。",
    ...
    {"command": "使用 [道具名] [数量]", "description": "使用消耗品或启用战备；经验书和双倍经验卡支持复数数量，双倍经验卡按生效场次累加；其他道具仍需逐个使用。"},
...
def _help_text() -> str:
    ...
    for item in _HELP_ITEMS:
        lines = [f"· {item['command']}", f"——{item['description']}"]
        for section in item.get("sections", []):
            lines.append(f"　· {section['title']}")
            lines.extend(f"　——{detail}" for detail in section["lines"])
        entries.append("\n".join(lines))
    body = "\n".join(entries)
