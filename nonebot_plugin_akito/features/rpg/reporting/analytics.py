...
from ..player.player import _level_of, _resolve_group
...
def _mark_rpg_first_seen(group: GroupRecord, user_ids: Iterable[object], today: str) -> None:
    ...
    if not isinstance(users, dict):
        return
    for user_id in user_ids:
        user = cast(dict[str, Any], users).get(str(user_id))
        ...
        if isinstance(raw_instances, list):
            for raw_instance in raw_instances:
                if isinstance(raw_instance, dict):
                    world_boss_instances.append(cast(WorldBossMetricRecord, dict(raw_instance)))
...
def _active_level_distribution(group: GroupRecord, today: str, period_days: int = 30) -> list[dict[str, Any]]:
    ...
    for uid in active_ids:
        ...
        if not isinstance(user, dict):
            continue
        level = _level_of(_safe_int(user.get("exp", 0)))
...
def _build_style_test_group(today: str) -> GroupRecord:
    ...
    return {"user_ids": user_ids, "users": cast(dict[str, Any], users), "rpg": {"metrics": {"days": days}}}
