...
└── features/                 # 独立功能模块（按功能分包）
    ...
    └── rpg/                  # RPG 子包：签到/打怪/小奇遇/世界BOSS/组队/强化/背包/群排行榜（详见 nonebot_plugin_akito/features/rpg/README.md）
                                   ...
                                   ├── player/player.py       # 经验→等级派生/称号/今日装备 helper/战力计算
                                   ├── signin/fortune.py      # 隐藏运势掷取（含连签保底）+ 签到钩子 on_signin
                                   ├── hunt/command.py        # 今日打怪指令 + 战斗结果播报组装
                                   ├── hunt/broadcast.py      # 普通战斗播报辅助
                                   ├── battle/combat.py       # 遭遇/精英/今日增益/胜负判定
                                   ├── battle/events.py       # 战斗事件/援护追击/小奇遇抽取
                                   ├── battle/friend_support.py # 群友助力候选与效果
                                   ├── battle/rewards/settlement.py # 经验积分掉落 + 单人/组队结算
                                   ├── battle/rewards/calculations.py # 基础奖励计算
                                   ├── battle/rewards/encounters.py # 支援与小奇遇奖励
                                   ├── utils.py          # 组队概率/战力与运势公共公式
                                   ├── simulation/runner.py     # 可复现的单人成长模拟
                                   ├── reporting/analytics.py   # 30日滚动统计 + 超管 RPG数据
                                   ├── world_boss/logic.py      # 世界BOSS状态、刷出与伤害逻辑
                                   ├── world_boss/settlement.py # 世界BOSS结算与跨日清理
                                   ├── world_boss/command.py    # 世界BOSS指令入口
                                   ├── team/team.py              # 组队@某人 指令
                                   ├── equipment/smith.py        # 强化/购买装备/重置RPG功能
                                   ├── supply/supply.py           # 每周冒险补给/阶梯成本/战备投放
                                   ├── inventory/inventory.py     # 背包/使用指令 + 道具效果 + 掉落 helper
                                   └── profile/character.py       # 我的角色面板 + 群排行榜 + 冒险帮助
```
...
- `handlers/` 和 `features/` 之间**无互相引用**；`features/rpg/` 对 `features/gift/` 有单向依赖：`team/team.py` / `world_boss/*` 消费羁绊等级，`inventory/inventory.py` 复用礼物结算，gift 不反向依赖 rpg，无环
- `features/scheduled/` 只允许调用 `features/rpg/world_boss/settlement.py` 的世界 BOSS 定时收尾 helper；不得扩散为 feature 之间的循环依赖
...
- `analysis.py`：Unicode/链接/CQ 清理、动态读取全部已注册 `on_command` 与 `on_regex` 规则并过滤整条命令消息、Jieba 分词、停用词及全局屏蔽词过滤、确定性排名
- `render.py`：WordCloud 词云和 HTMLRender 日报图片；词云采用橙/蓝/白色阶，消息饼图采用橙/蓝/灰色阶并用深灰表示“其他”（单人 TOP 5，其余合并为“其他”）；榜单头像加载失败时由模板降级为首字占位
- `commands.py`：`今日群聊词云` / `实时群聊词云` / `群聊词云 今天` 对目标群成员开放，并按群共享 30 分钟实时渲染冷却；历史日报查询、`重算群聊词云`、`测试群聊词云`、`词云帮助`、`查看/新增/移除词云屏蔽词` 和 `查看/新增/移除词云排除用户` 仍限 `SUPERUSER_QQ`；`回填群聊词云` 也限超管，指定当天时会导入历史快照并标记零点刷新
...
- 实时词云每次从当天原始消息重算，所以图库或派生模块新增命令入口无需同步维护词云黑名单；已保存日报需通过 `重算群聊词云 YYYY-MM-DD` 更新
...
| `player/player.py` | 纯函数：`_level_of(exp)` 经验→等级、称号、今日装备、战力计算与群校验 |
| `signin/fortune.py` | 签到钩子入口（暗掷运势、经验、装备与连签记录） |
| `hunt/command.py` | `今日打怪` 指令入口和普通战斗播报组装；掉落测试位于 `hunt/drop_test.py` |
| `battle/combat.py` | 战斗纯逻辑：遭遇分段、精英、今日增益与胜负判定 |
| `battle/events.py` | 普通/组队事件、援护追击和小奇遇抽取 |
| `battle/rewards/settlement.py` | 普通 RPG 的经验、积分、掉落、援护奖励和小奇遇入账；提供 `_settle_solo` / `_settle_coop` |
| `utils.py` | 普通组队与世界 BOSS 共用的组队成功率、协作战力加成、失败事件抽取及运势战力/掉落系数，避免两条战斗线重复维护同一公式 |
| `simulation/runner.py` | 使用生产战斗函数和独立随机源模拟成长；CLI 入口为 `py tools/simulate_rpg_growth.py` |
| `reporting/analytics.py` | 群级 30 日滚动 RPG 聚合数据与超管 `RPG数据` 看板 |
| `world_boss/logic.py` / `settlement.py` / `command.py` | 世界 BOSS 状态逻辑、结算清理和指令入口 |
| `team/team.py` | `组队@某人` 指令与羁绊事件，复用战斗结算模块 |
| `equipment/smith.py` | 强化、购买装备和重置 RPG 功能 |
| `supply/supply.py` | 每周冒险补给与阶梯成本 |
| `inventory/inventory.py` | `背包` / `使用` 指令、道具效果与掉落 helper |
| `profile/character.py` | `我的角色`、`群排行榜` 与 `冒险帮助` |

**依赖方向**（design constraint）：`features/gift/` 与 `features/rpg/*` 都依赖 `core/game_store.py`；签到走钩子表解耦（gift → `run_signin_hooks` → `signin/fortune.on_signin`，gift 不依赖 rpg）；三条 rpg→gift 单向依赖：`rpg/team/team.py` / `rpg/world_boss/*` → `gift._bond_level`（消费羁绊）、`rpg/inventory/inventory.py` → `gift._pick_gift_by_name`/`_settle`/`_build_broadcast` 等（礼物券消费走完整送礼流程）；gift 不反向依赖 rpg，无环。
...
- 冒险补给按全局玩家跨群共享周次数，满周 7 次共消耗 1200 积分、固定投放 210 经验。常规战备互斥，护符独立；组队各算各的战备，护符失败救场按发起人优先。远征套装不与双倍经验卡叠加或消耗卡片，所有战备都不得进入 `world_boss/` 世界 BOSS 链路。
